...
spec:
  ports:
    - port: 443
      targetPort: 8443
      nodePort: 32000 #增加nodePort: 32000
  type: NodePort
  selector:
    k8s-app: kubernetes-dashboard
...
或者
```
kubectl create serviceaccount dashboard-admin(账号) -n kubernetes-dashboard
kubectl create clusterrolebinding dashboard-admin(账号) --clusterrole=cluster-admin --serviceaccount=kubernetes-dashboard:dashboard-admin(账号)
```
两者效果一样的，只是账号不同
